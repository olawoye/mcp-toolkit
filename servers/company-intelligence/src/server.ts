...
export function createServer(config?: Partial<CompanyIntelligenceConfig>): McpServer {
  ...
  return {
    ...
    start() {
      ...
      const port = Number(process.env.MCP_PORT ?? process.env.PORT ?? 8160);
      ...
      const httpServer = http.createServer(async (req, res) => {
        ...
        if (req.method === 'GET' && requestUrl.pathname === '/health') {
          ...
          res.end(JSON.stringify({ ok: true, service: 'company-intelligence', tools: tools.map((t) => t.name), provider: resolved.provider }));
          return;
        }

        if (req.method === 'GET' && requestUrl.pathname === '/') {
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ name: 'company-intelligence', version: '0.1.0', tools: tools.map((t) => t.name), provider: resolved.provider }));
          return;
        }
      ...
      httpServer.listen(port, host, () => {
        logger.info('MCP company-intelligence HTTP server ready', {
          host,
          port,
